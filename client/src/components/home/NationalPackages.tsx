...
interface NationalPackageCardProps {
  packageData: Package;
}

const NationalPackageCard: React.FC<NationalPackageCardProps> = ({ packageData }) => {
...
export default function NationalPackages() {
  const { data: packages, isLoading, error } = useQuery<Package[]>({
    queryKey: ["/api/packages/national"],
  });

  return (
    <section className="py-16 bg-[#F5F5F5] hero-pattern" id="national">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12">
          <div>
            ...
            <p className="text-gray-600 max-w-2xl">
              Discover the incredible diversity of India with our curated
              national packages.
            </p>
          </div>
          <Link
            href="/package-list?type=national"
            className="mt-4 md:mt-0 text-[#000080] font-medium hover:text-[#FF9933] transition flex items-center"
          >
            View All National Packages
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="bg-white rounded-lg overflow-hidden shadow-lg h-[330px] animate-pulse"
              >
          ...
          <div className="text-center p-8 bg-red-50 rounded-lg text-red-500">
            Failed to load national packages. Please try again later.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
